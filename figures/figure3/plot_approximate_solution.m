  fid = fopen('../../k3/R10000/appb.dat');
  u1 = fread(fid,'double');
  status = fclose(fid);

  fid = fopen('../../k4/R10000/appb.dat');
  u2 = fread(fid,'double');
  status = fclose(fid);

  fid = fopen('../../k5/R10000/appb.dat');
  u3 = fread(fid,'double');
  status = fclose(fid);
  ...
  for m = 1:N
     ...
     % app4(:,1) = app4(:,1) + u4(m)*sin(m*s');    % u
     % app4(:,2) = app4(:,2) + u4(m)/(m^2)*sin(m*s');    % -\int\int u
  end
  ...
  % hold on;
  % plot(ax1, s,app4(:,1), 'Color', colors(4,:));
  hold off;
  grid on;
  title(ax1, "$u(x)$", 'Interpreter', 'latex');
  legend(ax1, "$k=3$", "$k=4$", "$k=5$", 'Interpreter', 'latex', 'Location','southeast');
  ...
  % hold on;
  % plot(ax2, s,app4(:,2), 'Color', colors(4,:)); 
  hold off;
  grid on;
  title(ax2, "$\psi(x)$", 'Interpreter', 'latex');
  legend(ax2, "$k=3$", "$k=4$", "$k=5$", 'Interpreter', 'latex', 'Location','southeast');
  ...
  exportgraphics(fig, 'figure3.pdf', 'ContentType', 'vector', 'BackgroundColor', 'none');