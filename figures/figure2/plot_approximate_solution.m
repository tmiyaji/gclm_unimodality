  fid = fopen('../../k2/R10/appb.dat');
  u1 = fread(fid,'double');
  status = fclose(fid);

  fid = fopen('../../k2/R100/appb.dat');
  u2 = fread(fid,'double');
  status = fclose(fid);

  fid = fopen('../../k2/R10000/appb.dat');
  u3 = fread(fid,'double');
  status = fclose(fid);
  ...
  legend(ax1, "$R=10$", "$R=10^2$", "$R=10^4$", 'Interpreter', 'latex', 'Location','northeast');
  ...
  legend(ax2, "$R=10$", "$R=10^2$", "$R=10^4$", 'Interpreter', 'latex', 'Location','northeast');
  ...
  exportgraphics(fig, 'figure2.pdf', 'ContentType', 'vector', 'BackgroundColor', 'none');